...
var zelcoreMarkets = {
  getAll() {
    return Promise.all([
      ...
      return apiRequest(`https://min-api.cryptocompare.com/data/pricemultifull?fsyms=${element}&tsyms=USD&api_key=${apiKey}`)//
      //  marketinfo coingecko
    })).concat(coingeckoIDs.map(async element => { 
      return apiRequest(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${element}&order=market_cap_desc&per_page=100&page=1&sparkline=false`)
    }))
    ).then((results) => {
      var markets = [];
      var cmk = {};
      ...
      dataCC.forEach((subresult) => {
        var coinsFull = Object.keys(subresult.RAW) 
