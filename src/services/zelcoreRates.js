...
var zelcoreRates = {
  getAll() {
    return Promise.all([
      ...
      apiRequest('https://min-api.cryptocompare.com/data/pricemulti?fsyms=TRX,DGB,XLM,DOGE,EOS,ADA,XRP,DOCK,NEO,TRON,BTT,SAFE,SUQA,BTH,GRS,XCASH,LEO,USDS&tsyms=BTC'),  //  3
      apiRequest('https://api.coingecko.com/api/v3/coins/markets?vs_currency=btc&ids=holotoken,half-life,axe,safe-coin-2,genesis-network,bzedge,commercium,bitcoin-zero,zelcash&order=market_cap_desc&per_page=100&page=1&sparkline=false'),  // 4
    ]).then((results) => {
      var rates = [];
      var efg = {};
      var errors = { errors: {} }
